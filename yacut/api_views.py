...
def map_short_id_to_url():
    ...
    custom_id = data['custom_id'] if 'custom_id' in data else None
    if custom_id and (
        len(custom_id) > MAX_SHORT_ID_LENGTH or
        not re.match(SHORT_ID_PATTERN, custom_id)
    ):
...
@app.route('/api/id/<string:short>', methods=['GET'], strict_slashes=False)
def get_url(short):
    query = URL_map.query.filter_by(short=short)
    if len(short) > MAX_SHORT_ID_LENGTH or not query.count():
        raise APIUsageError(SHORT_ID_NOT_FOUND, 404)
